...
public class GT_GUIContainer_ChargeOMat
extends GT_GUIContainerMetaID_Machine {
    public GT_GUIContainer_ChargeOMat(InventoryPlayer aInventoryPlayer, GT_TileEntity_ChargeOMat aTileEntity, int aID) {
        super(new GT_Container_ChargeOMat(aInventoryPlayer, aTileEntity, aID), (GT_TileEntityMetaID_Machine)aTileEntity, aID);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        int texture = this.mc.renderEngine.getTexture("/gregtechmod/textures/gui/ChargeOMat.png");
        ...
        if (this.mContainer != null) {
            int tScale = this.mContainer.mEnergy / Math.max(1, this.mContainer.mStorage / 141);
