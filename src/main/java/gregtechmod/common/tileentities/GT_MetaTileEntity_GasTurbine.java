...
public class GT_MetaTileEntity_GasTurbine extends MetaTileEntity {
	...
	@Override public int maxEUOutput()								{return 16;}
	@Override public int getInvSize()								{return 3;}
	@Override public int maxEUStore()								{return 1000000;}
	@Override public void onRightclick(EntityPlayer aPlayer)		{aPlayer.openGui(GT_Mod.instance, 118, mBaseMetaTileEntity.worldObj, mBaseMetaTileEntity.xCoord, mBaseMetaTileEntity.yCoord, mBaseMetaTileEntity.zCoord);}
    	...
    	if (!mBaseMetaTileEntity.worldObj.isRemote && mBaseMetaTileEntity.mTickTimer%10==0) {
    		...
    		if (mInventory[0] != null && mBaseMetaTileEntity.getStored() <= 100) {
    			for (int i = 0; i < GT_Recipe.sTurbineFuels.size(); i++) {
    				if (mInventory[0].isItemEqual(GT_Recipe.sTurbineFuels.get(i).mInput1)) {
    					if (LiquidContainerRegistry.getLiquidForFilledItem(mInventory[0]) == null) {
    						if (GT_Recipe.sTurbineFuels.get(i).mOutput1 != null) {
    							if (mInventory[1] == null) {
    								if (mBaseMetaTileEntity.increaseStoredEnergy(GT_Recipe.sTurbineFuels.get(i).mStartEU*1000)) {
    									mBaseMetaTileEntity.decrStackSize(0, 1);
    									mInventory[1] = GT_Recipe.sTurbineFuels.get(i).mOutput1.copy();
    								}
    							} else if (mInventory[1].isItemEqual(GT_Recipe.sTurbineFuels.get(i).mOutput1) && mInventory[1].stackSize + GT_Recipe.sTurbineFuels.get(i).mOutput1.stackSize <= mInventory[1].getMaxStackSize()) {
    								if (mBaseMetaTileEntity.increaseStoredEnergy(GT_Recipe.sTurbineFuels.get(i).mStartEU*1000)) {
        								mBaseMetaTileEntity.decrStackSize(0, 1);
    ...
    public int getFuelValue(LiquidStack aLiquid) {
    	...
    	LiquidStack tLiquid;
    	for (int i = 0; i < GT_Recipe.sTurbineFuels.size(); i++) {
    		if ((tLiquid = LiquidContainerRegistry.getLiquidForFilledItem(GT_Recipe.sTurbineFuels.get(i).mInput1)) != null) if (aLiquid.isLiquidEqual(tLiquid)) return GT_Recipe.sTurbineFuels.get(i).mStartEU;
        }
    	return 0;
    }
