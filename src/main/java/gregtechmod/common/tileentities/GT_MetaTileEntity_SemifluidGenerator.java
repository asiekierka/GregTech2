...
public class GT_MetaTileEntity_SemifluidGenerator extends MetaTileEntity {
	...
	@Override public void onRightclick(EntityPlayer aPlayer)		{aPlayer.openGui(GT_Mod.instance, 120, mBaseMetaTileEntity.worldObj, mBaseMetaTileEntity.xCoord, mBaseMetaTileEntity.yCoord, mBaseMetaTileEntity.zCoord);}
    					...
    					if (LiquidContainerRegistry.getLiquidForFilledItem(mInventory[0]) == null) {
    						if (GT_Recipe.sDenseLiquidFuels.get(i).mOutput1 != null) {
    							if (mInventory[1] == null) {
    								if (mBaseMetaTileEntity.increaseStoredEnergy(GT_Recipe.sDenseLiquidFuels.get(i).mStartEU*1000)) {
