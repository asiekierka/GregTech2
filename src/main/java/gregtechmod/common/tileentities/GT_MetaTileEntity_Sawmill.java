...
public class GT_MetaTileEntity_Sawmill extends MetaTileEntity {
	...
	@Override public void onRightclick(EntityPlayer aPlayer)		{aPlayer.openGui(GT_Mod.instance, 116, mBaseMetaTileEntity.worldObj, mBaseMetaTileEntity.xCoord, mBaseMetaTileEntity.yCoord, mBaseMetaTileEntity.zCoord);}
    ...
    private boolean checkMachine() {
    	...
    	for (int i = -1; i < 2; i++) for (int j = 1; j < 2; j++) for (int k = -1; k < 2; k++) {
            ...
            if (mBaseMetaTileEntity.worldObj.getBlockMetadata(mBaseMetaTileEntity.xCoord-xDir+i, mBaseMetaTileEntity.yCoord-yDir+j, mBaseMetaTileEntity.zCoord-zDir+k) != (i==0&&k==0?14:13)) return false;
    	}
    	return true;
    }
    ...
    private boolean checkRecipe() {
    	...
    	if (tRecipe != -1) {
    		if (spaceForOutput(tRecipe)) {
    			if (GT_Recipe.sSawmillRecipes.get(tRecipe).isRecipeInputEqual(true, true, mInventory[0], mInventory[1])) {
    		...
    		if (tRecipe != -1 && spaceForOutput(tRecipe)) {
    			if (GT_Recipe.sSawmillRecipes.get(tRecipe).isRecipeInputEqual(true, true, mInventory[0], tWaterStack)) {
		        	if (mInventory[0] != null) if (mInventory[0].stackSize == 0) mInventory[0] = null;
	...
	public int getInvSideLength(ForgeDirection aSide, int aFacing) {
		if (aSide == ForgeDirection.WEST || aSide == ForgeDirection.EAST) return 3;
		return 1;
	}
