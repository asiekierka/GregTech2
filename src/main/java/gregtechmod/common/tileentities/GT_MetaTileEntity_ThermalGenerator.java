...
public class GT_MetaTileEntity_ThermalGenerator
extends GT_MetaTileEntity_BasicTank {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        aPlayer.openGui((Object)GT_Mod.instance, 119, this.mBaseMetaTileEntity.worldObj, this.mBaseMetaTileEntity.xCoord, this.mBaseMetaTileEntity.yCoord, this.mBaseMetaTileEntity.zCoord);
    }
        ...
        block25: {
            ...
            if (this.mInventory[0] != null && this.mBaseMetaTileEntity.getStoredEnergy() <= 100) {
                ...
                for (int i = 0; i < GT_Recipe.sHotFuels.size(); ++i) {
                    if (!this.mInventory[0].isItemEqual(((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mInput1)) continue;
                    if (LiquidContainerRegistry.getLiquidForFilledItem((ItemStack)this.mInventory[0]) != null) break;
                    if (((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mOutput1 != null) {
                        if (this.mInventory[1] == null) {
                            if (!this.mBaseMetaTileEntity.increaseStoredEU(((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mStartEU * 1000, true)) break;
                            this.mBaseMetaTileEntity.decrStackSize(0, 1);
                            this.mInventory[1] = ((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mOutput1.copy();
                            break;
                        }
                        if (!this.mInventory[1].isItemEqual(((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mOutput1) || this.mInventory[1].stackSize + ((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mOutput1.stackSize > this.mInventory[1].getMaxStackSize() || !this.mBaseMetaTileEntity.increaseStoredEU(((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mStartEU * 1000, true)) break;
                        this.mBaseMetaTileEntity.decrStackSize(0, 1);
                        this.mInventory[1].stackSize += ((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mOutput1.stackSize;
                        break;
                    }
                    if (!this.mBaseMetaTileEntity.increaseStoredEU(((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mStartEU * 1000, true)) break;
                    this.mBaseMetaTileEntity.decrStackSize(0, 1);
    ...
    public int getFuelValue(LiquidStack aLiquid) {
        ...
        for (int i = 0; i < GT_Recipe.sHotFuels.size(); ++i) {
            LiquidStack tLiquid = LiquidContainerRegistry.getLiquidForFilledItem((ItemStack)((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mInput1);
            if (tLiquid == null || !aLiquid.isLiquidEqual(tLiquid)) continue;
            return ((GT_Recipe)GT_Recipe.sHotFuels.get((int)i)).mStartEU;
        }
        return 0;
    }
